...
/**
 * 중복 로그인 방지 implements HttpSessionListener, HttpSessionAttributeListener
 * 
 * @author ktm06
 *
 */
...
public class SessionEventListener implements HttpSessionListener, HttpSessionAttributeListener {
	...
	public boolean duplicatedSession(String u_id, String sessionId) {
		...
		Enumeration<Object> enumration = sessionRepogitory.elements();
		System.out.println("active session count : " + sessionRepogitory.size());
		System.out.println("u_id : " + u_id + "sessionId : " + sessionId);
		while (enumration.hasMoreElements()) {
			HttpSession registerSession = (HttpSession) enumration.nextElement();
			MemberVO vo = (MemberVO) registerSession.getAttribute("userInfo");

			if (vo != null) {

				if (vo.getU_id().equals(u_id)) {
					if (!registerSession.getId().equals(sessionId)) {
						System.out.println("duplicatedSession login - user : " + vo.getU_id() + " session id"
								+ registerSession.getId());
